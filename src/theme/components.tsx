...
export const Box = ({
  ...
  let focus, hover, srOnly, restOfStyles = null;
  if (styleSheet) {
...
export const Icon = ({ as, styleSheet, ...props }: ElementProps) => {
  let iconVariant, styleSheetUpdated = null;
  if (styleSheet) {
  ...
  const icon = iconSet[`fa${formattedIconVariant}`];
...
export const Button = ({ as, styleSheet, ...props }: ElementProps) => {
  ...
  let buttonVariant = 'primary', restStyleSheet = null;
  if (styleSheet) {
    let {
      buttonVariant,
      ...rest
